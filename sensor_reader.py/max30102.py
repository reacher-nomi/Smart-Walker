...
class MAX30102():
    ...
    def setup(self, led_mode=0x03):
        ...
        # Increased LED brightness for better heart rate detection
        # 0x1F = 6mA, 0x24 = 7mA, 0x32 = 10mA, 0x50 = 15mA, 0x7F = 25mA
        self.bus.write_i2c_block_data(self.address, REG_LED1_PA, [0x50])  # Red LED: 15mA (better signal)
        self.bus.write_i2c_block_data(self.address, REG_LED2_PA, [0x50])  # IR LED: 15mA (better signal)
        self.bus.write_i2c_block_data(self.address, REG_PILOT_PA, [0x7f])  # Pilot LED: 25mA (max)
    ...
    def read_temperature(self):
        ...
        self.bus.write_i2c_block_data(self.address, REG_TEMP_CONFIG, [0x01])
        
        # Wait for conversion to complete (typically 29ms)
        sleep(0.03)
        
        # Read temperature integer and fraction
        temp_int = self.bus.read_i2c_block_data(self.address, REG_TEMP_INTR, 1)[0]
        temp_frac = self.bus.read_i2c_block_data(self.address, REG_TEMP_FRAC, 1)[0]
        
        # Convert to Celsius
        # Integer part is signed, fraction is 0.0625 per LSB
        if temp_int > 127:
